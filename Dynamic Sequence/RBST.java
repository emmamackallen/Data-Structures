...
public class RBST {
	...
	*/
	private Node insertNormal(Node T, int team, int rank) {
		...
		if (rank <= rootNum) {
			T.setLeft(insertNormal(T.getRight(), team, rank));
		}
		else {
			T.setRight(insertNormal(T.getRight(), team, (rank - rootNum)));
		}
	...
	*/
	private Node[] split(Node T, int rank) {
		...
		if (rank < rootNum1) {
			...
			Node testNode = ret[1];
			ret[1] = T;
			ret[1].setLeft(testNode);
			ret[1].updateSize();
		}
		if (rank > rootNum1) {
			ret = split(T.getRight(), (rank - rootNum1));
			Node testNodeAgain = ret[0];
			ret[0] = T;
			ret[0].setRight(testNodeAgain);
			ret[0].updateSize();
	...
	*/
	private Node insert(Node T, int team, int rank) {
		...
		if (T.getLeft() == null) {
                        ...
                        rootNum2 = (T.getLeft().getSize() + 1);
                }

		if (randNum <= 1.0) {
			Node splitSet[] = split(T, (rank - 1));
		}			
		else if (rank <= rootNum2) {
			T.setLeft(insert(T.getLeft(), team, rank));
		}
		else {
			T.setRight(insert(T.getLeft(), team, (rank - rootNum2)));
		}
	...
	*/
	private Node select(Node T, int rank) {
		...
		return select(T.getRight(), (rank - rootNum3));	// Need to change this to return the correct node.
	}
